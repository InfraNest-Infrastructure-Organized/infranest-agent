...
This release was built with **Go __GOVERSION__**. Copy the block as it is:

```
git clone https://github.com/InfraNest-Infrastructure-Organized/infranest-agent
cd infranest-agent && git checkout __TAG__

GOTOOLCHAIN=__GOVERSION__ CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -trimpath \
...
Two things will make it *not* match, and both are easy to hit:

- **A different Go version.** Compiler output changes between releases, so `GOTOOLCHAIN` above is not
  decoration — it makes Go fetch and use the exact toolchain this was built with, whatever you have
  installed.
- **Building into the clone.** Go records VCS state in the binary, so an output path inside the working
  tree marks the module `+dirty` and changes the hash. `-o /tmp/...` is why the command writes there.

Set `GOOS`/`GOARCH` to whichever artefact you are checking.
